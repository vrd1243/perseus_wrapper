...
class PD:
        ...
        def plot_pd(self, filename):
                ...
                x_mor, y_mor, count_mor = self.mortal.T
                x_imm, count_imm = self.immortal.T
                y_imm = [self.lim for i in x_imm]
                x = np.linspace(0,self.lim+.2,10)
                
                count = np.concatenate([count_mor, count_imm], axis=0)
                min_, max_ = count.min(), count.max()
                fig = plt.figure(figsize=(11,9), dpi=100)
                plt.scatter(x_mor, y_mor, s=70, c=count_mor)
                plt.clim(min_, max_)
                plt.scatter(x_imm, y_imm, marker='^', s=120, c=count_imm)
                plt.clim(min_, max_)
                plt.colorbar()
                ax = plt.gca()
                ax.set_xlim(0,self.lim+.2)
                ax.set_ylim(0,self.lim+.2)
